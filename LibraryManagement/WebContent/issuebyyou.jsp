...
<body>
	...
	<h1 class="pagehead">Books Issued By You</h1>
	...
	<sql:setDataSource var="database" driver="com.mysql.cj.jdbc.Driver"
		url="jdbc:mysql://localhost:3306/library_management" user="root"
		password="password" />


	<sql:query dataSource="${database}" var="result">
         SELECT * from issued_books WHERE sid = ?;
         <sql:param value="${sc}" />
	</sql:query>
	...
	<div class="homeheading">
		...
		<c:forEach var="row" items="${result.rows}">
			...
			<div class="bookname"
				style="text-align: center; width: 580px; border-right: 2px solid black; padding: 13px 0px;">
				<c:out value="${row.bookname}" />
			</div>

			<div class="authorname"
				style="border-right: 2px solid black; text-align: center; width: 580px; padding: 13px 0px;">
				<c:out value="${row.authorname}" />
			</div>

			<div class="issuedby"
				style="border-right: 2px solid black; text-align: center; width: 150px; padding: 13px 0px;">
				<c:out value="${row.issuedby}" />
				(
				<c:out value="${row.sid}" />
				)
			</div>
			...
			<div class="issueddate"
				style="text-align: center; width: 130px; padding: 13px 0px;">
				<c:out value="${row.issueddate}" />
			</div>
		</c:forEach>
		</div>
</body>
